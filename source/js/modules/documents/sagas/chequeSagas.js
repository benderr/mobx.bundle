...
function* getListChequeSaga({isFirst = false, step = false}) {
	try {
		...
		if (dateFrom instanceof Date) {
			query.push(`beginDateTime=ge="${dateHelper.dateFormat(dateHelper.setStartDate(dateFrom), 'isoUtcDateTime')}"`);
		}
		if (dateTo instanceof Date) {
			query.push(`beginDateTime=le="${dateHelper.dateFormat(dateHelper.setEndDate(dateTo), 'isoUtcDateTime')}"`);
		}
		if (docType) {
