...
export default {
  ...
  getDayName(day, countChar) {
    const dayName = this.i18n[this.currentLocal].dayNames[day];
  ...
  getMonthName(monthNumber, declination, countChar) {
    const monthName = declination ? this.monthDeclinationRu(monthNumber) : this.i18n.ru.monthName[monthNumber - 1];
    if (!countChar && countChar > 0) {
  ...
  getCurrentWeekDates(date) {
    ...
    const delta = start.getDay() > 0 ? 1 : -6;
  ...
  getCurrentMonthDates(date) {
    ...
    const stop = new Date(now.getFullYear(), now.getMonth(), this.daysInMonth(now));
  ...
  getLastMonthDates(date) {
    ...
    const firstDayInlastMonthDate = new Date(now.getFullYear(), now.getMonth() - 1, 1);
    const stop = new Date(now.getFullYear(), now.getMonth() - 1, this.daysInMonth(firstDayInlastMonthDate));
  ...
  getLast30DaysDates(date) {
    ...
    const stop = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  ...
  getDateRange(date) {
    const start = this.setStartDate(new Date(date.getTime()));
    const stop = this.setEndDate(new Date(date.getTime()));
    return {
      startDate: start,
      stopDate: stop,
    };
  ...
  getCurrentYearDates(date) {
    ...
    const stop = new Date(now.getFullYear(), 11, 31);
  ...
  countCalendarDayBetween(date1, date2) {
    ...
    const _date1 = createOnlyDate(date1);
    const _date2 = createOnlyDate(date2);

    const dayCount = (_date1.getTime() - _date2.getTime()) / (24 * 60 * 60 * 1000);
  ...
  dateFormat(date, format, alternativeFormat, dateTimeNow) {
    let mask;
    let prefix = '';
    let token = /d{1,4}(?::?[C]?)|m{1,4}(?::?[IRCP]?)|yy(?:yy|y)?(?::?[IRC]?)|([HhMsTt])\1?|[LloSZ]|"[^"]*"|'[^']*'/g,
    ...
    const now = dateTimeNow || new Date();
    ...
    if (format == 'clever') {
      ...
      mask = alternativeFormat || `dd${ separator }mm${ separator }yyyy:C` + ' HH:MM';
      if (isCurrentYear) {
    ...
    if (format == 'cleverDate') {
      separator = this.i18n[this.currentLocal].dateSeparator;
      mask = alternativeFormat || `dd${ separator }mm${ separator }yyyy:C`;
      if (isCurrentYear) {
        mask = alternativeFormat || `dd${ separator }mm`;
      }
      if (isCurrentDay) {
    ...
    const parsedDate = mask.replace(token, ($0) => {
